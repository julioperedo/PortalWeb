...
(function($) {
        ...
        $(target).treegrid($.extend({}, opts, {
            ...
            loadFilter: function(data, parentId) {
                ...
                $(this).treegrid({
                    data: null,
                    url: null,
                    frozenColumns: [[$.extend({}, opts.frozenColumns[0][0], { title: opts.forzenColumnTitle })]],
                ...
                if (opts.showFooter) {
                    var footer = getFooter(this, filteredData);
    ...
    function getFooter(target, data) {
        var opts = $.data(target, 'pivotgrid').options;
        var footer = [];
        var fields = $(target).datagrid('getColumnFields');
        var temp = { children: [] };
        temp[$.fn.pivotgrid.defaults.treeField] = opts.FooterTitle;
        $.map(fields, function(field) {
            temp[field] = _sum(field);
        });
        footer.push(temp);
        return footer;
        ...
        function _sum(field) {
            ...
            var rr = $.map(data, function(row) {
                for (var i = 0; i < opts.pivot.columns.length; i++) {
                    if ((i < (col.tt.length - 1)) & col.tt[i] != opts.FooterTitle) {
    ...
    function getRows(target, data) {
        ...
        $.map(opts.pivot.rows, function(field, index) {
            ...
            var xfield = Enumerable.From(opts.pivot.xfields).Where("$.field == '" + (typeof field == "object" ? field.field : field) + "'").FirstOrDefault();
            xfield = xfield || { state: "closed" };
            if (pfield) {
                var tmpRows = [];
                while (topRows.length) {
                    ...
                    $.map(groups, function(rows) {
                        ...
                        if (index < (opts.pivot.rows.length - 1)) { r.state = xfield.state; }
                        allRows.push(r);
                        tmpRows.push(r);
                    })
                }
                ...
                //groups = Enumerable.From(data).GroupBy("$." + field, null, function(key, g) { return g.ToArray(); }).ToArray();
                $.map(groups, function(rows) {
                    var r = sumR1(rows);
                    r._rows = rows;
                    r[opts.treeField] = rows[0][field];
        ...
        function getR1(field, rows) {
            rows = Enumerable.From(rows).OrderBy("$." + field).ToArray();
            var result = {};
            $.map(rows, function(row) {
                var val = row[field];
                var rr = result[val];
    ...
    function getColumns(target, data) {
            ...
            if (pcolumns) {
                ...
                $.map(pcolumns, function(pcol) {
                    ...
                    $.map(subcol, function(v) {
                        cc.push({ _field: field, title: v, tt: pcol.tt.concat(v), colspan: opts.pivot.values.length });
                    });
                ...
                $.map(getV1(field), function(v) {
                    cc.push({ _field: field, title: v, tt: [v], colspan: opts.pivot.values.length });
                });
                columns.push(cc);
            }
        });
        if (columns.length == 0) {
            columns.push([{ _field: opts.FooterTitle, title: opts.FooterTitle, tt: [opts.FooterTitle], colspan: opts.pivot.values.length }]);
        }

        var cc = [];
        $.map(columns[columns.length - 1], function(col, index) {
            $.map(opts.pivot.values, function(v) {
                var xfield = Enumerable.From(opts.pivot.xfields).Where("$.field == '" + v.field + "'").FirstOrDefault();
                if (xfield == null) { xfield = { formatter: null, styler: null, title: null, op: null }; }
                cc.push($.extend({}, v, {
                    field: col.tt.join('_') + '_' + v.field,
                    title: (xfield.title || (v.title || v.field)),
                    tt: col.tt.concat(v.field),
                    width: (v.width || opts.valueFieldWidth),
                    align: (v.align || 'right'),
                    styler: (xfield.styler || (v.styler || opts.valueStyler)),
                    formatter: (xfield.formatter || (v.formatter || opts.valueFormatter)),
                    op: xfield.op,
                    sortable: true,
                    sorter: function(a, b) {
                        var v1 = parseFloat(a);
        ...
        function getV1(field, pfield, pvalue) {
            var lstResult = [];
            if (pfield == undefined || pvalue == undefined) {
                lstResult = Enumerable.From(data).OrderBy("$." + field).Select("$." + field).Distinct().ToArray();
            } else {
                lstResult = Enumerable.From(data).OrderBy("$." + field).Where("$." + pfield + " == '" + pvalue + "'").Select("$." + field).Distinct().ToArray();
            }
            if (lstResult.length != 1 & opts.showColumnsTotals) {
                lstResult.push(opts.FooterTitle);
            }
            return lstResult;
        }
    ...
    function layout(target) {
        ...
        function fill(p, d) {
            ...
            $.map(d, function(name) {
                ...
                item.linkbutton($.extend({}, opts, { text: text, plain: true, width: '100%' }));
            });
        }
        function dnd() {
    ...
    $.fn.pivotgrid.defaults = $.extend({}, $.fn.treegrid.defaults, {
        ...
        frozenColumns: [[{ field: '_tree_field', width: 350, title: '', sortable: true }]],
        autoRowHeight: false,
        remoteSort: false,
        forzenColumnTitle: '',
        valueFieldWidth: 80,
